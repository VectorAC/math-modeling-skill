...
开始写作前，逐项**完整阅读**以下材料。阅读标题、文件名、历史对话或模型记忆均不算完成阅读；清单未读完不得开始写作。

1. `references/2401445-paper-analysis.md` — 美赛 O 奖论文五维度分析（摘要/结构/图表/写作/模型）
2. 与本题同题型的获奖论文分析 2-3 篇（均为 md 文本，位于 `math_model/国赛美赛指导/`）：
   - 国赛：`国赛论文结构性指导-2025{A,B,C,D,E}.md`
   - 美赛：`美赛论文结构性指导-{论文编号}.md`（20 余篇可选）
3. 规范底稿：国赛见 `references/cumcm-template/cumcm2025_template.tex` 的章节结构；美赛见 `references/contest-diff.md`

（PDF 原论文不参与本清单——多模态读取不可用，只读文本版分析。）
...
- [ ] 写作前必读清单是否逐项读毕？语言锁定声明是否已输出且全程一致？
- [ ] AI 交互记录是否完整可用？
