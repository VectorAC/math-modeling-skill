...
| 0.5 | 题目文件一次性抽取为信息卡（extract_input.py，乱码走 OCR 兜底链） | agent-prompts/phase05-problem-info.md | 原文完整无删改；存疑项已处理并确认 | 用户核对信息卡 | problem_info |
| 1 | 拆解/假设/数据需求/硬约束逐字摘录/思考路径叙事 | agent-prompts/phase1-analysis.md | 思考路径四步齐全有转折点；硬约束逐字不转述 | 出口交互门（强制） | 子问题/假设/硬约束 |
| 2 | 2-3 方案两风格对比/选定+数学表达/创新点陈述 | agent-prompts/phase2-model-design.md | ≥2 方案且说 Why；创新点陈述已确认 | 方案前/后交互门 | 选定方案+创新点 |
...
- 打磨与解释空间可在两个窗口同时开启，通过 `.claude/math-modeling/space-chat.md` 跨会话互通（规则见两空间文件）
...
核心：subagent-driven-development / dispatching-parallel-agents / verification-before-completion（缺失时按 phase0-init.md 报告格式提示）。外部：analyze-mcm-paper（/analyze-mcm-paper <论文路径>）、officecli/pdftotext/pypdf/pandas+xlrd（题目抽取，见 phase05 模板与 extract_input.py）、OCR 兜底（pdftoppm+tesseract，乱码率>5% 时直跳）。
...
| 维度 | 国赛 | 美赛 |
|------|------|------|
| 语言/摘要 | 中文摘要+关键词 | 英文 Summary Sheet 250-350 词 |
| 算法描述 | 流程图+文字，伪代码附录 | Algorithm 环境可接受 |
| 引用 | GB/T 7714，中英结合 | 英文为主，15 篇左右 |
| 加分项 | 真实数据、政策建议 | Summary Sheet、跨场景泛化 |
| 页数 | 正文 30 页左右（3 问 28-32/5 问 30-35，非硬门禁） | ≤25 页，Letter |
